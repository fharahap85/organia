    ...
    <div className="min-h-screen bg-slate-950 text-white flex">
        ...
        <div>
          ...
          <nav className="p-4 space-y-1">
            {sidebarItems.map((item) => {
              const isActive = item.path === '/kaderisasi';
              return (
                <Link
                  key={item.name}
                  to={item.path}
      ...
      <div className="flex-grow flex flex-col min-w-0">
        <header className="h-16 border-b border-slate-800 bg-slate-900/50 px-8 flex items-center shrink-0">
          <h1 className="text-xl font-bold">Jenjang Kaderisasi</h1>
        </header>
        <main className="p-8 overflow-y-auto flex-grow space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center shadow-xl max-w-xl mx-auto">
            <Award className="mx-auto text-indigo-400 mb-4" size={56} />
            <h2 className="text-2xl font-bold mb-2">Jenjang Kaderisasi</h2>
            <p className="text-slate-400 text-sm">Halaman ini akan segera hadir untuk mengelola jenjang dan tahapan kaderisasi.</p>
          </div>
        </main>
      </div>
    </div>
